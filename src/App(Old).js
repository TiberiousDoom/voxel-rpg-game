...
const VoxelRPG = () => {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.key === 'h' || e.key === 'H') {
        // Use potion logic directly in event handler
        setInventory(prev => {
          if (prev.potions > 0 && player.health < player.maxHealth) {
            setPlayer(p => ({ ...p, health: Math.min(p.maxHealth, p.health + 50) }));
            showMessage('Health restored!');
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (deltaTime < 3) { // Only update if delta is reasonable (prevent spiral of death)
        gameLoop();
      }
  ...
  }, [gameState, keys, mousePos, player.x, player.y, player.level, player.defense, showBase, bosses.length, inDungeon, dungeons, gainXP, pickupLoot, updateQuest]);
  ...
  useEffect(() => {
    ...
    // Optimized terrain rendering - batch by type
    ...
    // Draw each terrain type in batches
    ...
    // Optimized loot rendering with culling
    loot.forEach(item => {
    ...
    // Optimized particles with culling
    particles.forEach(p => {
    ...
    // Optimized projectiles with culling and reduced shadow effects
    ...
    // Optimized enemies with culling
